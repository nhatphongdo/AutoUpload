...
namespace PhotoshopFile
{
    public class Mask
    {
        ...
        public MagickImage ToImage(Color color)
        {
            var image = new MagickImage(MagickColor.FromRgba(0, 0, 0, 0), Rect.Width, Rect.Height);
            var pixels = new byte[ImageData.Length * 4];
            for (var y = 0; y < Rect.Height; y++)
                for (var x = 0; x < Rect.Width; x++)
                {
                    pixels[4 * (x + y * Rect.Width)] = color.R;
                    pixels[4 * (x + y * Rect.Width) + 1] = color.G;
                    pixels[4 * (x + y * Rect.Width) + 2] = color.B;
                    pixels[4 * (x + y * Rect.Width) + 3] = ImageData[x + y * Rect.Width];
                }
            image.GetPixels().Set(pixels);
